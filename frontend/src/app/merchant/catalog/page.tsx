...
export default function CatalogPage() {
  ...
  const [sku, setSku] = useState(""); const [name, setName] = useState(""); const [price, setPrice] = useState(""); const [msg, setMsg] = useState("");
  ...
  async function add(e: React.FormEvent) {
    ...
    try {
      ...
      setSku(""); setName(""); setPrice(""); setMsg(""); load();
        ...
        {products.length === 0 && !loaded ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border2">
              {products.map((p) => (
                <tr key={p.id}>
                  <td className="py-2.5"><b>{p.name}</b> <span className="ml-1 text-xs text-muted">{p.sku}</span></td>
